...
def test(data,
    ...
    # Print results
    print('***TEST*** Class: {}, Images: {}, Targets: {}, Prec: {:.4f}, Recall: {:.4f}, mAP@.5: [{:.4f}], '
          'mAP@.5:.95: {:.4f}'.format('all', seen, nt.sum(), mp, mr, map50, map))
...
if __name__ == '__main__':
    ...
    elif opt.task == 'study':  # run over a range of settings and /plot
        for weights in ['yolov5s.pt', 'yolov5m.pt', 'yolov5l.pt', 'yolov5x.pt']:
            f = 'study_%s_%s.txt' % (Path(opt.data).stem, Path(weights).stem)  # filename to  to
